...
const execCmd = (command, source = overworld, ignoreError = false) => {
    try {
...
class objective {
    ...
    static delete = (id) => !execCmd(`scoreboard objectives remove ${toExecutable(id)}`, undefined, true).statusCode;
...
class dummies {
    ...
    #obj;
    #data;
    "set" = (name, score) => (void execCmd(`scoreboard players set ${toExecutable(name)} ${this.#obj.execId} ${score}`, undefined, true), this);
    add = (name, score) => (void execCmd(`scoreboard players add ${toExecutable(name)} ${this.#obj.execId} ${score}`, undefined, true), this);
    "get" = (name) => {
    ...
    exist = (name) => !execCmd(`scoreboard players test ${toExecutable(name)} ${this.#obj.execId} * *`, undefined, true).statusCode;
    delete = (name) => !execCmd(`scoreboard players reset ${toExecutable(name)} ${this.#obj.execId}`, undefined, true).statusCode;
    getScores = (() => {
        const t = this;
        return function* () {
            for (const { participant, score } of t.#data.getScores())
...
class display {
    static setDisplay = (displaySlot, scoreboard) => !execCmd(`scoreboard objectives setdisplay ${displaySlot} ${scoreboard instanceof objective ? scoreboard.execId : toExecutable(scoreboard)}`, undefined, true).statusCode;
    static clearDisplay = (displaySlot) => !execCmd(`scoreboard objectives setdisplay ${displaySlot}`, undefined, true).statusCode;
    constructor(key, obj) {
        if (key !== auth)
            throw new TypeError("Class is not constructable");
        this.#obj = obj;
    ...
    #obj;
    setDisplay = (displaySlot) => !execCmd(`scoreboard objectives setdisplay ${displaySlot} ${this.#obj.execId}`, undefined, true).statusCode;
