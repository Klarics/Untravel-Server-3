...
if (config.debug) {
    for (const k in world.events) {
        const evSignal = world.events[k], OSubscribe = evSignal.subscribe.bind(evSignal);
        OSubscribe((evd) => {
            ...
            for (const [fn, { stack }] of fnList) {
                ...
                try {
                    ...
                    console.warn(`world > events > ${k} (${fn.name || "(anonymous)"} (${fsource})): ${e instanceof Error ? `${e}\n${e.stack}\n${stack}` : e}`);
                }
                const t1 = Date.now();
        ...
        evSignal.subscribe = (fn) => {
            if (!fn)
                return console.warn(`Event ${k} - 'fn' argument not passed`);
        ...
        evSignal.unsubscribe = (fn) => {
            ...
            if (!fnList.delete(fn))
                console.warn(`Failed to unsubscribe ${fn.name || "(anonymous)"} (${fn.fileName}:${fn.lineNumber}) from event ${k}`);
        };
    }
}
