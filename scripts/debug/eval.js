...
export function evalCmd(evd, args, argFull) {
    ...
    evd.cancel = true;
    // check for toggle
    if (!(config.debug && enableEval))
        return sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r Eval is disabled.`);
    ...
    if (hash === undefined || encode !== hash) {
        return sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r You need to be Paradox-Opped to use this command.`);
    }
    // tells staff
    player.addTag("evalSelf");
    sendMsg("@a[tag=paradoxOpped, tag=!evalSelf]", `§r§4[§bUntravel§eMx§4]§r ${player.nameTag}§r is using §6Eval§r: ${argFull}`);
...
const objSelfProperty = new Proxy(Object.setPrototypeOf({
