...
function illegalitemsb(object) {
    ...
    if (antiShulkerBoolean && item.id in shulkerItems) {
        ...
        sendMsgToPlayer(source, `§r§4[§bUntravel§eMx§4]§r Shulker Boxes are not allowed!`);
        return;
    }
    ...
    if (item.amount < 0 || item.amount > maxStack) {
        ...
        try {
            let invContainer = source.getComponent("minecraft:inventory");
            invContainer.container.setItem(hand, new ItemStack(MinecraftItemTypes.air, 0));
            sendMsg("@a[tag=notify]", `§r§4[§bUntravel§eMx§4]§r ${source.nameTag}§r detected with stacked items greater than x${maxStack} for '${itemId}'.`);
            sendMsgToPlayer(source, `§r§4[§bUntravel§eMx§4]§r Stacked item '${itemId}' cannot exceed x${maxStack}!`);
        }
        catch (error) { }
        if (stackBanBoolean) {
            // Ban
    ...
    if (illegalLoresBoolean && !config.modules.illegalLores.exclude.includes(String(item.getLore()))) {
        ...
        sendMsg("@a[tag=notify]", `§r§4[§bUntravel§eMx§4]§r Removed ${item.id.replace("minecraft:", "")} with lore from ${source.nameTag}.`);
        sendMsgToPlayer(source, `§r§4[§bUntravel§eMx§4]§r Item with illegal lores are not allowed!`);
        return;
    }
    if (illegalEnchantmentBoolean) {
        // We get a list of enchantments on this item
        let enchantComponent = item.getComponent("minecraft:enchantments");
            ...
            for (let { level, type: { id }, } of item_enchants) {
                ...
                if (!enchantLevel) {
                    ...
                    sendMsg("@a[tag=notify]", [
                        `§r§4[§bUntravel§eMx§4]§r §4[§f${source.nameTag}§4]§r §6=>§r §4[§fSlot§4]§r ${hand}§r §6=>§r §4[§f${item.id.replace("minecraft:", "")}§4]§r §6Enchanted: §4${id}=${level}§r`,
                        `§r§4[§bUntravel§eMx§4]§r Removed §4[§f${item.id.replace("minecraft:", "")}§4]§r from ${source.nameTag}.`,
                    ]);
                    sendMsgToPlayer(source, `§r§4[§bUntravel§eMx§4]§r Illegal enchantments are not allowed!`);
