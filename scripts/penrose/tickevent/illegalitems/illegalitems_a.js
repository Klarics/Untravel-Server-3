...
const World = world;
function rip(player, inventory_item, enchData) {
...
function illegalitemsa() {
    ...
    let illegalItemsABoolean = World.getDynamicProperty("illegalitemsa_b"), salvageBoolean = World.getDynamicProperty("salvage_b"), illegalLoresBoolean = World.getDynamicProperty("illegallores_b"), illegalEnchantmentBoolean = World.getDynamicProperty("illegalenchantment_b"), antiShulkerBoolean = World.getDynamicProperty("antishulker_b"), stackBanBoolean = World.getDynamicProperty("stackban_b");
    ...
    for (let player of World.getPlayers()) {
        ...
        let hash = player.getDynamicProperty("hash"), salt = player.getDynamicProperty("salt"), encode;
        try {
        ...
        if (hash !== undefined && encode === hash)
            continue;
        let inventory = player.getComponent("minecraft:inventory"), container = inventory.container;
        for (let i = 0; i < container.size; i++) {
            ...
            if (antiShulkerBoolean && inventory_item.id in shulkerItems) {
                ...
                sendMsg("@a[tag=notify]", `§r§b-§5§lUntravel§eMx§b-r Removed ${inventory_item.id.replace("minecraft:", "")} from ${player.nameTag}.`);
                sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-r Shulker Boxes are not allowed!`);
                continue;
            }
            if (salvageBoolean && inventory_item.id in whitelist === false) {
                 ...
                 */
                let enchantArray = [], enchantLevelArray = [], verifiedItemName = inventory_item.nameTag, newNameTag = titleCase(inventory_item.id.replace("minecraft:", "")), actualItemName = new ItemStack(Items.get(inventory_item.id));
            ...
            if (inventory_item.amount < 0 || inventory_item.amount > maxStack) {
                ...
                sendMsg("@a[tag=notify]", `§r§b-§5§lUntravel§eMx§b-r ${player.nameTag}§r detected with stacked items greater than x${maxStack} for '${itemId}'.`);
                sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-r Stacked item '${itemId}' cannot exceed x${maxStack}!`);
                if (stackBanBoolean) {
                    // Ban
                    return rip(player, inventory_item, null);
                }
            ...
            if (illegalLoresBoolean && !config.modules.illegalLores.exclude.includes(String(inventory_item.getLore()))) {
                ...
                sendMsg("@a[tag=notify]", `§r§b-§5§lUntravel§eMx§b-r Removed ${inventory_item.id.replace("minecraft:", "")} with lore from ${player.nameTag}.`);
                sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-r Item with illegal lores are not allowed!`);
                continue;
            }
            if (illegalEnchantmentBoolean) {
                // We get a list of enchantments on this item
                let enchantContainer = inventory_item.getComponent("minecraft:enchantments"), item_enchants = enchantContainer.enchantments, 
                // List of allowed enchantments on item
                        ...
                        switch (true) {
                            ...
                            case level > enchantLevel || level < 0: {
                                ...
                                sendMsg("@a[tag=notify]", [
                                    `§r§b-§5§lUntravel§eMx§b-r §4[§f${player.nameTag}§4]§r §6=>§r §4[§fSlot§4]§r ${i}§r §6=>§r §4[§f${inventory_item.id.replace("minecraft:", "")}§4]§r §6Enchanted: §4${id}=${level}§r`,
                                    `§r§b-§5§lUntravel§eMx§b-r Removed §4[§f${inventory_item.id.replace("minecraft:", "")}§4]§r from ${player.nameTag}.`,
                                ]);
                                sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-r Illegal enchantments are not allowed!`);
