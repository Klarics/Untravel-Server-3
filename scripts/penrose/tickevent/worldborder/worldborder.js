import { BlockLocation, Location, MinecraftBlockTypes, world } from "mojang-minecraft";
    ...
    for (let player of World.getPlayers()) {
        ...
        if (player.dimension.id === "minecraft:nether") {
            ...
            if (player.location.x > netherSize || player.location.x < -netherSize || player.location.z > netherSize || player.location.z < -netherSize) {
                sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r You have reached the world border.`);
                // Positives
