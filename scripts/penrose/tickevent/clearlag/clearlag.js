...
function executionItem() {
    ...
    for (let entity of World.getDimension("overworld").getEntities(filter)) {
        ...
        try {
            let itemContainer = entity.getComponent("item");
...
function clearlag(id) {
    ...
    if (activeTimer[0] === 1 && activeTimer[2] === "minutes" && activeTimer[1] <= 0) {
        ...
        sendMsg("@a", `§r§4[§bUntravel§eMx§4]§r Server lag will be cleared in 60 seconds!`);
    }
    // Give advance warning
    if (activeTimer[0] === 5 && activeTimer[1] === "seconds") {
        // Notify 10 seconds in advance
        sendMsg("@a", `§r§4[§bUntravel§eMx§4]§r Server lag will be cleared in 5`);
    }
    // Give advance warning
    if (activeTimer[0] === 4 && activeTimer[1] === "seconds") {
        // Notify 10 seconds in advance
        sendMsg("@a", `§r§4[§bUntravel§eMx§4]§r 4`);
    }
    // Give advance warning
    if (activeTimer[0] === 3 && activeTimer[1] === "seconds") {
        // Notify 5 seconds in advance
        sendMsg("@a", `§r§4[§bUntravel§eMx§4]§r 3`);
    }
    // Give advance warning
    if (activeTimer[0] === 2 && activeTimer[1] === "seconds") {
        // Notify 5 seconds in advance
        sendMsg("@a", `§r§4[§bUntravel§eMx§4]§r 2`);
    }
    // Give advance warning
    if (activeTimer[0] === 1 && activeTimer[1] === "seconds") {
        // Notify 5 seconds in advance
        sendMsg("@a", `§r§4[§bUntravel§eMx§4]§r 1`);
    }
    ...
    if (activeTimer[0] <= 0) {
        ...
        sendMsg("@a", `§r§4[§bUntravel§eMx§4]§r Server has been cleared to reduce lag!`);
    }
    else {
