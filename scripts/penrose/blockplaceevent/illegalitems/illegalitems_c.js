...
const World = world;
function rip(player, inventory_item, enchData, block) {
    if (!enchData && !block) {
...
function illegalitemsc(object) {
    ...
    if (antiShulkerBoolean && block.type.id in shulkerItems) {
        sendMsg("@a[tag=notify]", `§r§4[§bUntravel§eMx§4]§r Removed ${block.type.id.replace("minecraft:", "")} from ${player.nameTag}.`);
        sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r Shulker Boxes are not allowed!`);
        // Set block in world
    ...
    let inventory;
    try {
        let invComponent = block.getComponent("inventory");
    ...
    if (inventory) {
        for (let i = 0; i < inventory.size; i++) {
            ...
            if (inventory_item.amount < 0 || inventory_item.amount > maxStack) {
                ...
                sendMsg("@a[tag=notify]", `§r§4[§bUntravel§eMx§4]§r ${player.nameTag}§r detected with stacked items greater than x${maxStack} for '${itemId}'.`);
                sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r Stacked item '${itemId}' cannot exceed x${maxStack}!`);
                if (stackBanBoolean) {
                    return rip(player, inventory_item, null, null);
                }
                try {
            ...
            if (illegalLoresBoolean && !config.modules.illegalLores.exclude.includes(String(inventory_item.getLore()))) {
                ...
                sendMsg("@a[tag=notify]", `§r§4[§bUntravel§eMx§4]§r Removed ${block.type.id.replace("minecraft:", "")} with lore from ${player.nameTag}.`);
                sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r Item with illegal lores are not allowed!`);
                continue;
            }
            if (illegalEnchantmentBoolean) {
                // We get a list of enchantments on this item
                let enchantComponent = inventory_item.getComponent("minecraft:enchantments");
                    ...
                    for (let { level, type: { id }, } of item_enchants) {
                        ...
                        if (!enchantLevel) {
                            ...
                            sendMsg("@a[tag=notify]", [
                                `§r§4[§bUntravel§eMx§4]§r §4[§f${player.nameTag}§4]§r §6=>§r §4[§f${block.type.id.replace("minecraft:", "")}§4]§r §6=>§r §4[§fSlot§4]§r ${i}§r §6=>§r §4[§f${inventory_item.id.replace("minecraft:", "")}§4]§r §6Enchanted: §4${id}=${level}§r`,
                                `§r§4[§bUntravel§eMx§4]§r Removed §4[§f${inventory_item.id.replace("minecraft:", "")}§4]§r from ${player.nameTag}.`,
                            ]);
                            sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r Illegal enchantments are not allowed!`);
            ...
            if (block.type.id in whitelistChest) {
                ...
                sendMsg("@a[tag=notify]", `§r§4[§bUntravel§eMx§4]§r ${player.nameTag} placed a nested chest at X=${x.toFixed(0)}, Y=${y.toFixed(0)}, Z=${z.toFixed(0)}. Chest has been cleared!`);
                sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r Nested chests are not allowed. This chest has been cleared!`);
