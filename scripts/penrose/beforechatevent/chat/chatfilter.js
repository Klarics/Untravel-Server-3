    ...
    World.events.beforeChat.subscribe((msg) => {
            ...
            for (let player of World.getPlayers()) {
                if (player.hasTag("tpr:" + msg.sender.name)) {
                    ...
                    if (cooldownCalc === msSettings || cooldownCalc <= 0) {
                         ...
                         */
                        sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r Hello ${player.name}! ${msg.sender.name} has approved your request to teleport to their location!`);
                        sendMsgToPlayer(msg.sender, `§r§4[§bUntravel§eMx§4]§r You have approved a teleport request from ${player.name}!`);
            ...
            for (let player of World.getPlayers()) {
                if (player.hasTag("tpr:" + msg.sender.name)) {
                    ...
                    if (cooldownCalc === msSettings || cooldownCalc <= 0) {
                         ...
                         */
                        sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r Hello ${player.name}! ${msg.sender.name} has denied your request to teleport to their location!`);
                        sendMsgToPlayer(msg.sender, `§r§4[§bUntravel§eMx§4]§r You have denied a teleport request from ${player.name}!`);
                        player.removeTag("tpr:" + msg.sender.name);
                        msg.sender.removeTag("tpr");
                        break;
                    }
        ...
        if (chatRanksBoolean === true) {
            ...
            if (player.hasTag("isMuted")) {
                sendMsgToPlayer(player, `§r§4[§bUntravel§eMx§4]§r You are currently muted.`);
                msg.cancel = true;
                return;
            }
            ...
            if (!msg.cancel) {
                ...
                sendMsg("@a", `§r§7${player.name}§7[§3${rank}§r§7]: §r${message}`);
                msg.cancel = true;
            }
        }
        else if (!msg.cancel) {
