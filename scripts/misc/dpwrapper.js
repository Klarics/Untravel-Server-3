...
if (config.dynamicPropertyWrapper.enabled) {
    ...
    const uniqueID = dpw === "3#fa52" ? String(crypto(dpw, pw)).slice(0, 6) : dpw;
    const getPropertyDatas = (scoreObj) => {
        ...
        for (const [name] of scoreObj.getScores()) {
            ...
            if (typeof match == "undefined")
                continue;
            // parse value
            const valueStr = name.substring(match.length), value = type == "number" ? +valueStr : type == "boolean" ? valueStr == "true" : JSON.parse(`"${valueStr}"`);
    ...
    Player.prototype.setDynamicProperty = function (id, value) {
        ...
        if (propData.scoreboardName !== undefined)
            obj.delete(propData.scoreboardName);
        // set
        propData.cachedValue = value;
