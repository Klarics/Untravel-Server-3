...
function clearTickTimeout(handle) {
    ...
    if (!tickTimeoutMap.delete(handle))
        console.warn(`Failed to clear tick timeout with ID ${handle}: the ID doesn't exist`);
}
/**
...
function clearTickInterval(handle) {
    ...
    if (!tickIntervalMap.delete(handle))
        console.warn(`Failed to clear tick interval with ID ${handle}: the ID doesn't exist`);
}
