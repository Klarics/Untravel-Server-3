...
function mayflydisable(player, member) {
    sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled fly mode for ${player === member ? "themselves" : member.nameTag}.`);
}
function mayflyenable(player, member) {
    sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled fly mode for ${player === member ? "themselves" : member.nameTag}.`);
}
/**
 * @name fly
...
export function fly(message, args) {
    ...
    if (!membertag.includes("noflying") && !membertag.includes("flying")) {
        try {
            ...
            return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r Education Edition is disabled in this world.`);
        }
        return;
    }
