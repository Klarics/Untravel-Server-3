...
export function report(message, args) {
    ...
    if (!member) {
        sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r ${prefix}report <player> <reason>§r`);
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r Couldnt find that player!`);
    }
    // Make sure they dont report themselves
    if (member === player) {
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r You cannot report yourself.`);
    }
    sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r Reported ${member.nameTag}§r with reason: ${reason}`);
    sendMsg("@a[tag=notify]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has reported ${member.nameTag}§r with reason: ${reason}`);
}
