...
function goHomeHelp(player, prefix) {
    ...
    return sendMsgToPlayer(player, [
        `\n§b-§dCommand§b-§r: gohome`,
        `§b-§dStatus§b-§r: ${commandStatus}`,
        `§b-§dUsage§b-§r: gohome [optional]`,
        `§b-§dOptional§b-§r: name, help`,
        `§b-§dDescription§b-§r: Return home to a specified saved location.`,
        `§b-§dExamples§b-§r:`,
        `    ${prefix}gohome barn`,
        `    ${prefix}gohome help`,
    ]);
...
export function gohome(message, args) {
    ...
    if (!homex || !homey || !homez || !dimension) {
        ...
        if (cooldownCalc === msSettings || cooldownCalc <= 0 || (hash !== undefined && encode === hash)) {
            ...
            sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r Welcome back!`);
            ...
            // Teleporting to fast
            sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r Too fast! Please wait for ${activeTimer} before going home.`);
        }
    }
}
