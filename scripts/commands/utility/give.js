/* eslint no-var: "off"*/
import { world, MinecraftItemTypes, ItemStack } from "mojang-minecraft";
...
export function give(message, args) {
    ...
    if (confirmItem) {
        ...
        if (maxStack >= Number(args[2])) {
            ...
            inv.addItem(item);
            return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r ${member.name} was given ${args[1]} x${args[2]}.`);
        }
        else {
            return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r This stack is too high! ${maxStack} is the max. Try again.`);
        }
    }
    else {
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r This item could not be found! Try again.`);
    }
}
