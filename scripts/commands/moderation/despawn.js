...
export function despawn(message, args) {
    ...
    if (counter > 0 && verify === false) {
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r Despawned ${requestedEntity} (x${counter})!`);
    }
    if (verify === true) {
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r Despawned all entities (x${counter})!`);
    }
    // If nothing then abort and let them know
    if (args[0] !== "all" && args.length > 0) {
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r This entity was not found in the world!`);
    }
    else {
        // Need to give a parameter that is recognized
        return sendMsgToPlayer(player, [`§r§b-§5§lUntravel§eMx§b-§r Please specify which entity or target all!`, `§r§b-§5§lUntravel§eMx§b-§r Example: ${prefix}despawn iron_golem`, `§r§b-§5§lUntravel§eMx§b-§r Example: ${prefix}despawn all`]);
    }
}
