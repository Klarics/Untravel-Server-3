...
export function op(message, args) {
    ...
    if ((hash === undefined || (hash !== encode && args[0] !== config.modules.encryption.password)) && !player.hasTag('Adminer')) {
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r You need to be §eServer-Opped§r to use this command.`);
    }
    else if (hash === encode && args[0] === config.modules.encryption.password && player.hasTag('Adminer')) {
        // Old stuff that makes up for less than 5% of the project
        sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r You are now §eServer op§r!`);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r is now §eServer-Opped§r.`);
    ...
    sendMsgToPlayer(member, `§r§b-§5§lUntravel§eMx§b-§r You are now op!`);
    sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${member.nameTag}§r is now Paradox-Opped.`);
    member.addTag("paradoxOpped");
    member.addTag('staffstatus');
    member.addTag('Ambassador');
}
