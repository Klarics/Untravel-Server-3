...
function listQueue(queue, player) {
    if (queue) {
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r is queued to be unbanned`);
    }
}
...
export function unban(message, args) {
    ...
    sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${args.join(" ").replace(regexp, "")} is queued to be unbanned!`);
}
