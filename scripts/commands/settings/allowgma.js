...
export function allowgma(message, args) {
    ...
    if (adventureGMBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disallowed §4Gamemode 2 (Adventure)§r to be used!`);
        Adventure();
    }
    else if (adventureGMBoolean === true) {
        // Deny
        World.setDynamicProperty("adventuregm_b", false);
        return sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has allowed §6Gamemode 2 (Adventure)§r to be used!`);
    }
}
