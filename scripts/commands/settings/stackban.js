...
export function stackban(message, args) {
    ...
    if (!illegalItemsABoolean && !illegalItemsBBoolean && !illegalItemsCBoolean) {
        ...
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r You need to enable Illegal Items to use this feature.`);
    }
    if (stackBanBoolean === false) {
        // Allow
        World.setDynamicProperty("stackban_b", true);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6StackBans§r!`);
        return;
    }
    else if (stackBanBoolean === true) {
        // Deny
        World.setDynamicProperty("stackban_b", false);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4StackBans§r!`);
        return;
    }
}
