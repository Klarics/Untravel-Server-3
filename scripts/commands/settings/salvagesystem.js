...
export function salvage(message, args) {
    ...
    if (salvageBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6Salvage§r!`);
        return;
    }
    else if (salvageBoolean === true) {
        // Deny
        World.setDynamicProperty("salvage_b", false);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4Salvage§r!`);
        return;
    }
}
