...
const World = world;
function spammerDHelp(player, prefix, spammerDBoolean) {
    let commandStatus;
...
export function spammerD(message, args) {
    ...
    if (spammerDBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6SpammerD§r!`);
        SpammerD();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4SpammerD§r!`);
        return;
    }
}
