...
export function allowgms(message, args) {
    ...
    if (survivalGMBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disallowed §4Gamemode 0 (Survival)§r to be used!`);
        Survival();
        return;
    }
        ...
        // Deny
        World.setDynamicProperty("survivalgm_b", true);
        return sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has allowed §6Gamemode 0 (Survival)§r to be used!`);
    }
}
