...
export function illegalitemsB(message, args) {
    ...
    if (illegalItemsBBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6IllegalItemsB§r!`);
        IllegalItemsB();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4IllegalItemsB§r!`);
        return;
    }
}
