...
export function autokillaura(message, args) {
    ...
    if (autoaurascore <= 0) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6Autoaura§r!`);
    }
    else if (autoaurascore >= 1) {
        // Deny
        player.runCommand(`scoreboard players set paradox:config autoaura 0`);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4Autoaura§r!`);
    }
