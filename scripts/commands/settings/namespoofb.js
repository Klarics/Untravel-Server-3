...
export function namespoofB(message, args) {
    ...
    if (nameSpoofBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6NamespoofB§r!`);
        NamespoofB();
        return;
    }
        ...
        // Deny
        World.setDynamicProperty("namespoofb_b", true);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4NamespoofB§r!`);
        return;
    }
}
