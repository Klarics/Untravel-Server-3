...
export function autoclick(message, args) {
    ...
    if (autoclickerscore <= 0) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6Anti Autoclicker§r!`);
    }
    else if (autoclickerscore >= 1) {
        // Deny
        player.runCommand(`scoreboard players set paradox:config autoclicker 0`);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4Anti Autoclicker§r!`);
    }
