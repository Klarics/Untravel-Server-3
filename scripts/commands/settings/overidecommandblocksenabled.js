...
export function overidecommandblocksenabled(message, args) {
    ...
    if (cmdsscore <= 0) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has set CommandBlocksEnabled as §6enabled§r!`);
    }
    else if (cmdsscore === 1) {
        // Deny
        player.runCommand(`scoreboard players set paradox:config cmds 2`);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has set CommandBlocksEnabled as §4disabled§r!`);
    }
    else if (cmdsscore >= 2) {
        // Force
        player.runCommand(`scoreboard players set paradox:config cmds 0`);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has §etoggled§r Force-CommandBlocksEnabled!`);
    }
