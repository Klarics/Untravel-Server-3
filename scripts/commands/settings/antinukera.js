...
export function antinukerA(message, args) {
    ...
    if (antiNukerABoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6AntiNukerA§r!`);
        NukerA();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4AntiNukerA§r!`);
        return;
    }
}
