...
const World = world;
function crasherbHelp(player, prefix, crasherBBoolean) {
    let commandStatus;
 ...
 */
export function crasherB(message, args) {
    ...
    if (crasherBBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6CrasherB§r!`);
        CrasherB();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4CrasherB§r!`);
        return;
    }
}
