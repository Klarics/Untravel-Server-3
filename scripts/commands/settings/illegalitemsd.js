...
export function illegalitemsD(message, args) {
    ...
    if (illegalItemsDBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6IllegalItemsD§r!`);
        IllegalItemsD();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4IllegalItemsD§r!`);
        return;
    }
}
