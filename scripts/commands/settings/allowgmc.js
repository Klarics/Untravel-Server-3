...
export function allowgmc(message, args) {
    ...
    if (creativeGMBoolean === false) {
        ...
        if (adventureGMBoolean === true && survivalGMBoolean === true) {
            ...
            sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r Since all gamemodes were disallowed, Adventure mode has been enabled.`);
            Adventure();
            return;
        }
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disallowed §4Gamemode 1 (Creative)§r to be used!`);
        Creative();
        return;
    }
    else if (creativeGMBoolean === true && player.hasTag('Adminer')) {
        // Deny
        World.setDynamicProperty("creativegm_b", false);
        return sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has allowed §6Gamemode 1 (Creative)§r to be used!`);
    }
}
