...
export function invalidsprintA(message, args) {
    ...
    if (invalidSprintABoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6InvalidSprintA§r!`);
        InvalidSprintA();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4InvalidSprintA§r!`);
        return;
    }
}
