...
export function badpackets2(message, args) {
    ...
    if (badPackets2Boolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6Badpackets2§r!`);
        BadPackets2();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4Badpackets2§r!`);
        return;
    }
}
