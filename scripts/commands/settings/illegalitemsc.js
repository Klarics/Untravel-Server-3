...
export function illegalitemsC(message, args) {
    ...
    if (illegalItemsCBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6IllegalItemsC§r!`);
        IllegalItemsC();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4IllegalItemsC§r!`);
        return;
    }
}
