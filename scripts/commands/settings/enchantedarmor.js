...
export function enchantedarmor(message, args) {
    ...
    if (encharmorscore <= 0) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6Anti Enchanted Armor§r!`);
    }
    else if (encharmorscore >= 1) {
        // Deny
        player.runCommand(`scoreboard players set paradox:config encharmor 0`);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4Anti Enchanted Armor§r!`);
    }
