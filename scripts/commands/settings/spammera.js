...
export function spammerA(message, args) {
    ...
    if (spammerABoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6SpammerA§r!`);
        SpammerA();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4SpammerA§r!`);
        return;
    }
}
