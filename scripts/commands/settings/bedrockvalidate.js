...
export function bedrockvalidate(message, args) {
    ...
    if (bedrockValidateBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6BedrockValidate§r!`);
        BedrockValidate();
        return;
    }
        ...
        return sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4BedrockValidate§r!`);
    }
}
