...
export function worldborders(message, args) {
    ...
    if ((hash === undefined || encode !== hash) && !player.hasTag('Adminer')) {
        return sendMsgToPlayer(player, `§r§b-§5§lUntravel§eMx§b-§r You need to be §6Server-Opped§r to use this command.`);
    }
    ...
    if (argCheck === "disable") {
        // Disable Worldborder
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled the §6World Border§r!`);
    ...
    if ((args[0] === "-overworld" || args[0] === "-o") && isNaN(Number(args[1])) === false && (args[2] === "-nether" || args[2] === "-n") && isNaN(Number(args[3])) === false) {
        // Build the wall
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has set the §6World Border§r! Overworld: ${args[1]} Nether: ${args[3]}`);
    ...
    if ((args[0] === "-nether" || args[0] === "-n") && isNaN(Number(args[1])) === false && (args[2] === "-overworld" || args[2] === "-o") && isNaN(Number(args[3])) === false) {
        // Build the wall
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has set the §6World Border§r! Overworld: ${args[3]} Nether: ${args[1]}`);
        World.setDynamicProperty("worldborder_b", true);
    ...
    if (isNaN(Number(args[0])) === false && isNaN(Number(args[1])) === false) {
        // Build the wall
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has set the §6World Border§r! Overworld: ${args[0]} Nether: ${args[1]}`);
        World.setDynamicProperty("worldborder_b", true);
    ...
    if ((args[0] === "-nether" || args[0] === "-n") && isNaN(Number(args[1])) === false) {
        // Build the wall
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has set the §6World Border§r! Nether: ${args[1]}`);
        World.setDynamicProperty("worldborder_b", true);
        World.setDynamicProperty("worldborder_nether_n", Math.abs(Number(args[1])));
        WorldBorder();
        return;
    ...
    if ((args[0] === "-overworld" || args[0] === "-o") && isNaN(Number(args[1])) === false) {
        // Build the wall
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has set the §6World Border§r! Overworld: ${args[1]}`);
        World.setDynamicProperty("worldborder_b", true);
        World.setDynamicProperty("worldborder_n", Math.abs(Number(args[1])));
        WorldBorder();
        return;
