...
export function illegalEnchant(message, args) {
    ...
    if (illegalEnchantmentBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6IllegalEnchantments§r!`);
        return;
    }
    else if (illegalEnchantmentBoolean === true) {
        // Deny
        World.setDynamicProperty("illegalenchantment_b", false);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4IllegalEnchantments§r!`);
        return;
    }
}
