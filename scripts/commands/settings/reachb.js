...
export function reachB(message, args) {
    ...
    if (reachBBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6ReachB§r!`);
        ReachB();
        return;
    }
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4ReachB§r!`);
        return;
    }
}
