...
export function antishulker(message, args) {
    ...
    if (antiShulkerBoolean === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has enabled §6Anti-Shulkers§r!`);
        return;
    }
    else if (antiShulkerBoolean === true) {
        // Deny
        World.setDynamicProperty("antishulker_b", false);
        sendMsg("@a[tag=paradoxOpped]", `§r§b-§5§lUntravel§eMx§b-§r ${player.nameTag}§r has disabled §4Anti-Shulkers§r!`);
        return;
    }
}
